...
int main(int argc, char* argv[])
{
    ...
    INFO("添加单个用户信息");
    {
        try
        {
            ...
            //开始创建userIdentityRole对象
            auto curUserIdentityRole = std::make_shared<suiUserIdentityRole::UserIdentityRole>(dbHandler, rehandler, curRemoveCache);

            //开始添加
            curUserIdentityRole->insert("user_id_sui", suiDataSql::roleType::roleTypeAdmin, suiDataSql::identityType::identityTypeAdmin);
            t.commit();
            INFO("添加成功, 点击回车继续运行");
            std::cin.get();
            INFO("开始测试查询功能");
...

    {
        try
        {
            ...
            //开始查询
            auto ret = curUserIdentityRole->select("user_id_sui");
            t.commit();
            if(!ret)
            {
                //不存在
                ERROR("查询失败, 用户身份角色信息不存在");
                return -1;
            }
            //存在
            INFO("查询成功, 用户身份角色信息存在, 角色为: {}， 身份为： {}", suiDataSql::suiRoleType::roleTypeToString(ret->getRoleType())
                                            , suiDataSql::suiIdentityType::identityTypeToString(ret->getIdentityType()));
    ...
    {  
        try
        {
            ...
            //开始删除
            curUserIdentityRole->remove("user_id_sui");
            t.commit();
        }
        catch (const odb::exception& e)
        {
